...
?>
	<div id="admin_center_block">
		<p id="center_title"><a href="index.php">Главная</a> / товары</p>
		<?php
...
if($_SESSION['chk_products'] == '1') {
			...
			$result = mysql_query("SELECT * FROM table_products $query_start_num",$link);
				if (mysql_num_rows($result) > 0) {
					$row = mysql_fetch_array($result);
					$count = mysql_num_rows($result);
					?>
		<p id="center_header">Общее количество товаров - <span><?php echo $count;?></span><a href="new_product.php">Добавить</a><form id="search_form" action="product_search.php" method="GET"><label for="product_key" id="label_product"></label><input type="text" name="product_key" id="product_key" placeholder="Поиск по коду товара" /><input type="submit" id="go_search" value="Поиск"/></form></p>
		...
		<?php 
							...
							$height = intval($ratio*$height);
								...
								echo '
									...
									<li>
										...
										<div class="product_title"><a href="../product/'.$row["products_id"].'" class="product_name">'.$row["title"].'</a></div>
			...
			if ($total > 1) {
				echo '
					...
					$count = 0;
					echo '
					<p id="center_header">Общее количество товаров - <span>'.$count.'</span><a href="new_product.php">Добавить</a></p>
